...
int main()
{
	// LibOVR: initialise and requests information about the available HMD
	// -------------------------------------------------------------------
	ovrResult result = ovr_Initialize(nullptr);
	if (OVR_FAILURE(result)) {
		std::cout << "LibOVR: Unable to initialise." << std::endl;
	...
	if (OVR_FAILURE(result))
	{
		std::cout << "LibOVR: Unable to create session." << std::endl;
		ovr_Shutdown();
		std::cout << "Press ENTER to exit." << std::endl;
		getchar();
		return 0;
	...
	// LibOVR: querying the HMD for ts current tracking state
	// ------------------------------------------------------
	...
	// load textures
	// -------------
	unsigned int cubeTexture = loadTexture((const char*)(resourcePath + "textures/container.jpg").c_str());
